...
module AdditionalIssuePermissionsPlugin
  module IssuePatch
    ...
    module InstanceMethods
      ...
      def edit_issues_permission(user=User.current)
        self.new_record? ||
        ((self.author == user) && user.allowed_to?(:edit_issues_4author, self.project)) ||
        ((self.assigned_to == user) && user.allowed_to?(:edit_issues_4assigned_to, self.project)) ||
